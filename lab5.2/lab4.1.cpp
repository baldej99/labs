...
int main() {
    ...
    //delete axe;
    magicStaff.Attack();
    DisposableWeapon grenade;
    grenade.Attack();
    grenade.Attack();
    WeaponPosition<MagicWeapon, DisposableWeapon> holder(magicStaff, grenade);
    cout << "Оружие в левой руке: " << holder.getLeftHand().getName() << endl;
    cout << "Оружие в правой руке: " << holder.getRightHand().getName() << endl;
    return 0;
}
